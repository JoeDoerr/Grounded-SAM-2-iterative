...
import io
from sam2.sam2_video_predictor import _load_img_bytes_as_tensor

def first_step(text, first_frame_list):
    """
    ...
    inference_state = video_predictor.non_video_path_init_state(single_image_bytes=first_frame_list)#video_predictor.init_state(video_path=video_dir) #FIX
    ...
    image = Image.open(io.BytesIO(frame_names[0]))
    ...
    for _, segments in video_segments.items():
        ...
        return video_predictor, inference_state, masks
...
def update_inference_state(inference_state, frame, video_predictor):
    ...
    new_image = _load_img_bytes_as_tensor(frame, video_predictor.image_size)
    inference_state["images"] = torch.cat([inference_state, new_image], dim=0)
...
def new_frame(video_predictor, inference_state, new_frame):
    ...
    video_segments = {}  # video_segments contains the per-frame segmentation results
    for out_frame_idx, out_obj_ids, out_mask_logits in video_predictor.propagate_in_video(inference_state, start_frame_idx=inference_state_idx):
        video_segments[out_frame_idx] = {
            out_obj_id: (out_mask_logits[i] > 0.0).cpu().numpy()
            for i, out_obj_id in enumerate(out_obj_ids)
        }
